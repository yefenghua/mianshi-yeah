...
public class SentinelRulesManager {
    ...
    public void initFlowRules() {
        ...
        ParamFlowRule ruleUserLogin = new ParamFlowRule("userLogin")
                .setParamIdx(0) // 对第 0 个参数限流，即 IP 地址
                .setCount(10) // 每分钟最多 10 次
                .setDurationInSec(60); // 规则的统计周期为 60 秒
        ParamFlowRuleManager.loadRules(Arrays.asList(rule,ruleUserLogin));
    }
    ...
    public void initDegradeRules() {
        ...
        // 单 IP 查看题目列表熔断规则
        DegradeRule slowCallRuleUserLogin = new DegradeRule("userLogin")
                .setGrade(CircuitBreakerStrategy.SLOW_REQUEST_RATIO.getType())
                .setCount(0.2) // 慢调用比例大于 20%
                .setTimeWindow(60) // 熔断持续时间 60 秒
                .setStatIntervalMs(30 * 1000) // 统计时长 30 秒
        ...
        DegradeRule errorRateRuleUserLogin = new DegradeRule("userLogin")
                .setGrade(CircuitBreakerStrategy.ERROR_RATIO.getType())
                .setCount(0.1) // 异常率大于 10%
        ...
        // 加载规则
        DegradeRuleManager.loadRules(Arrays.asList(slowCallRule, errorRateRule,slowCallRuleUserLogin, errorRateRuleUserLogin));
    }
